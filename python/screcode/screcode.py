...
class RECODE():
    ...
    def _noise_variance_stabilizing_normalization(
        ...
        self.log_['#silent %ss' % self.unit] = X.shape[1] - \
            sum(self.idx_sig) - sum(self.idx_nonsig)
        return X_norm
    ...
    def fit_transform(self, X):
        ...
        recode_ = RECODE_core(variance_estimate=False, fast_algorithm=self.fast_algorithm,
        ...
        X_RECODE[:, self.idx_nonsilent] = self._inv_noise_variance_stabilizing_normalization(
            X_norm_RECODE)
        ...
        X_RECODE_ss = (np.median(np.sum(X_RECODE[:, self.idx_nonsilent], axis=1)) *
                       X_RECODE[:, self.idx_nonsilent].T/np.sum(X_RECODE[:, self.idx_nonsilent], axis=1)).T
        self.cv_ = np.zeros(X.shape[1], dtype=float)
        self.cv_[self.idx_nonsilent] = np.std(
            X_RECODE_ss, axis=0)/np.mean(X_RECODE_ss, axis=0)
    ...
    def check_applicability(
        ...
        spec = matplotlib.gridspec.GridSpec(
            ncols=2, nrows=1, width_ratios=[4, 1], wspace=0.)
        ax0 = fig.add_subplot(spec[0])
        ax0.scatter(x[idx_sig], y[idx_sig], color='b', s=ps,
                    label='significant %s' % self.unit, zorder=2)
        ax0.scatter(x[idx_nonsig], y[idx_nonsig], color='r', s=ps,
                    label='non-significant %s' % self.unit, zorder=3)
        ...
        ax0.legend(loc='upper left', borderaxespad=0, fontsize=14,
                   markerscale=2).get_frame().set_alpha(0)
        ylim = ax0.set_ylim()
        ax1 = fig.add_subplot(spec[1])
        sns.kdeplot(y=np.log10(
            norm_var[norm_var > 0]), color='k', shade=True, ax=ax1)
        ...
        dens = scipy.stats.kde.gaussian_kde(
            np.log10(norm_var[norm_var > 0]))(x)
        peak_val = x[np.argmax(dens)]
        rate_low_var = np.sum(
            norm_var[norm_var > 0] < 0.90)/len(norm_var[norm_var > 0])
        ...
        if (rate_low_var < 0.01) and (np.abs(peak_val) < 0.1):
            ...
            backcolor = 'tomato'
        ax0.text(0.99, 0.982, applicability, va='top', ha='right',
                 transform=ax0.transAxes, fontsize=14, backgroundcolor=backcolor)
    ...
    def plot_procedures(
        self,
        titles=('Original data', 'Normalized data', 'Projected data',
                'Variance-modified data', 'Denoised data'),
        figsize=(7, 5),
        save=False,
        save_filename='RECODE_procedures',
        save_filename_foots=('1_Original', '2_Normalized',
                             '3_Projected', '4_Variance-modified', '5_Denoised'),
        save_format='png',
        dpi=None,
        show=True
    ):
    ...
    def plot_original_data(
        ...
        plt2 = ax.scatter(x, y2, color='r', s=ps,
                          label='Noise', zorder=2, marker='x')
        ...
        ax.set_yscale('log')
        ax.legend(loc='upper left', borderaxespad=0, fontsize=14,
                  markerscale=5, handletextpad=0.).get_frame().set_alpha(0)
        plt1.set_alpha(0.1)
        plt2.set_alpha(0.1)
    ...
    def plot_normalized_data(
        ...
        idx_sort = np.argsort(np.mean(X_scaled, axis=0))
        x = np.arange(X_scaled.shape[1])
        ax.scatter(x, np.var(X_norm, axis=0, ddof=1)[
                   idx_sort], color='k', s=ps, zorder=2)
    ...
    def plot_projected_data(
        ...
        idx_sort = np.argsort(np.mean(X_scaled, axis=0))
        x = np.arange(X_scaled.shape[1])
        ax.scatter(np.arange(n_EV)+1, plot_EV, color='k',
                   label='Original', s=ps, zorder=1)
    ...
    def plot_variance_modified_data(
        ...
        idx_sort = np.argsort(np.mean(X_scaled, axis=0))
        x = np.arange(X_scaled.shape[1])
        ax.scatter(np.arange(n_EV)+1, plot_EV, color='lightblue',
                   label='Original', marker='^', s=ps, zorder=1)
        ax.scatter(np.arange(n_EV)+1, plot_EV_mod, color='k',
                   label='Modified', s=ps, zorder=2)
        ax.axvline(self.recode_.ell, color='gray', ls='--')
        ax.text(self.recode_.ell*1.1, 0.2, '$\ell$=%d' %
                self.recode_.ell, color='k', fontsize=16, ha='left')
        ax.set_xlabel('PC', fontsize=fs_label)
        ax.set_ylabel('Variance (eigenvalue)', fontsize=fs_label)
        # ax.set_xscale('log')
        ax.set_yscale('symlog')
        ax.set_ylim([0, max(plot_EV)*1.5])
        ax.legend(loc='upper right', borderaxespad=0, fontsize=14,
                  markerscale=2, handletextpad=0.).get_frame().set_alpha(0)
        ax.set_title(title, fontsize=fs_title)
        plt.gca().spines['right'].set_visible(False)
        plt.gca().spines['top'].set_visible(False)
        if save:
    ...
    def plot_denoised_data(
        ...
        plt1 = ax.scatter(x, y1, color='lightblue', s=ps,
                          label='Original', zorder=1, marker='^')
        plt2 = ax.scatter(x, y2, color='k', s=ps,
                          label='Denoised', zorder=2, marker='o')
        ax.set_ylim([min(min(y1), min(y2))*0.5, max(max(y1), max(y2))])
        ax.set_yscale('log')
        ax.set_xlabel(self.Unit, fontsize=fs_label)
        ax.set_ylabel('Variance', fontsize=fs_label)
        ax.legend(loc='upper left', borderaxespad=0, fontsize=14,
                  markerscale=7, handletextpad=0.).get_frame().set_alpha(0)
        plt1.set_alpha(0.05)
        plt2.set_alpha(0.1)
        ax.set_title(title, fontsize=fs_title)
        plt.gca().spines['right'].set_visible(False)
    ...
    def plot_mean_variance(
        ...
        if size_factor == 'median':
            ...
            size_factor_RECODE = np.median(np.sum(self.X_RECODE, axis=1))
        X_ss_log = np.log2(
            size_factor*(self.X[:, self.idx_nonsilent].T/np.sum(self.X, axis=1)).T+1)
        X_RECODE_ss_log = np.log2(
            size_factor_RECODE*(self.X_RECODE[:, self.idx_nonsilent].T/np.sum(self.X_RECODE, axis=1)).T+1)
        fig, ax0 = plt.subplots(figsize=figsize)
        plt.rcParams['xtick.direction'] = 'in'
        plt.rcParams['ytick.direction'] = 'in'
        x, y = np.mean(X_ss_log, axis=0), np.var(X_ss_log, axis=0, ddof=1)
        ax0.scatter(x, y, color='b', s=ps, label='significant %s' %
                    self.unit, zorder=2)
        ...
        plt.gca().spines['top'].set_visible(False)
        if save:
            plt.savefig('%s_Original.%s' %
                        (save_filename, save_format), dpi=dpi)
        fig, ax1 = plt.subplots(figsize=figsize)
        x, y = np.mean(X_RECODE_ss_log, axis=0), np.var(
            X_RECODE_ss_log, axis=0, ddof=1)
        ax1.scatter(x, y, color='b', s=ps, label='significant %s' %
                    self.unit, zorder=2)
    ...
    def plot_mean_cv(
        ...
        fs_label = 14
        fs_title = 14
        X_ss = (np.median(np.sum(self.X[:, self.idx_nonsilent], axis=1))*self.X[:,
                self.idx_nonsilent].T/np.sum(self.X[:, self.idx_nonsilent], axis=1)).T
        fig, ax0 = plt.subplots(figsize=figsize)
        plt.rcParams['xtick.direction'] = 'in'
        plt.rcParams['ytick.direction'] = 'in'
        ...
        X_RECODE_ss = (np.median(np.sum(self.X_RECODE[:, self.idx_nonsilent], axis=1))*self.X_RECODE[:,
                       self.idx_nonsilent].T/np.sum(self.X_RECODE[:, self.idx_nonsilent], axis=1)).T
        fig, ax1 = plt.subplots(figsize=figsize)
        ...
        if show_features:
            ...
            detect_rate = np.sum(np.where(self.X > 0, 1, 0), axis=0)[
                self.idx_nonsilent]/self.X.shape[0]
            idx_detect_rate_n = detect_rate <= cut_detect_rate
            idx_detect_rate_p = detect_rate > cut_detect_rate
            ax1.scatter(x[idx_detect_rate_n], cv[idx_detect_rate_n], color='gray', s=ps,
                        label='detection rate <= {:.2%}'.format(cut_detect_rate), alpha=0.5)
            ax1.scatter(x[idx_detect_rate_p], cv[idx_detect_rate_p], color='b', s=ps,
                        label='detection rate > {:.2%}'.format(cut_detect_rate), alpha=0.5)
            ax1.legend(loc='upper center', bbox_to_anchor=(
                0.5, -0.15), ncol=2, fontsize=12, markerscale=2)
            idx_rank_cv = np.argsort(cv[idx_detect_rate_p])[::-1]
            texts = [plt.text(x[idx_detect_rate_p][idx_rank_cv[i]], cv[idx_detect_rate_p][idx_rank_cv[i]],
                              index[self.idx_nonsilent][idx_detect_rate_p][idx_rank_cv[i]], color='red') for i in range(n_show_features)]
            adjustText.adjust_text(
                texts, arrowprops=dict(arrowstyle='->', color='k'))
        else:
    ...
    def plot_ATAC_preprocessing(
        ...
        plt.rcParams['xtick.direction'] = 'in'
        plt.rcParams['ytick.direction'] = 'in'
        plt.plot(val[1:], count[1:], color='lightblue',
                 zorder=1, marker='^', label='Original')
        ...
        plt.plot(val[1:], count[1:], color='gray',
                 marker='o', label='Preprpcessed', zorder=3)
...
class RECODE_core():
    ...
    def fit(self, X):
        ...
        svd = sklearn.decomposition.TruncatedSVD(
            n_components=self.n_pca).fit(X_svd-X_svd_mean)
        ...
        for i in range(len(PCA_Ev_NRM)-1):
            PCA_Ev_NRM[i] -= (np.sum(self.PCA_Ev[i+1:]) +
                              PCA_Ev_sum_diff)/(n_Ev_all-i-1)
    ...
    def _noise_reduct_param(
        ...
        self.X_RECODE = _noise_reductor(
            self.X, self.L, self.U, self.X_mean, self.ell)
    ...
    def _noise_reduct_noise_var(
        ...
        PCA_Ev_sum = np.array([np.sum(self.PCA_Ev[i:])
                              for i in range(self.n_pca)])+PCA_Ev_sum_diff
    ...
    def _noise_var_est(
        ...
        for i in range(num_div_max):
            ...
            for j in range(num_div):
                ...
                k[j] = len(np.where((X_var_sub < div_max)
                           & (X_var_sub > div_min))[0])
        ...
        idx_set_k_max = np.where((X_var_sub < div_max)
                                 & (X_var_sub > div_min))[0]
